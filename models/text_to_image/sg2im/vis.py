...
def draw_scene_graph(objs, triples, path, vocab=None, **kwargs):
  """
  ...
  if vocab != None:
    for key, value in vocab['pred_name_to_idx'].items():
      reversed_vocab[value] = key
  ...
  if vocab is not None:
    ...
    for i in range(triples.size(0)):
      s = triples[i, 0].item()
      p = vocab['pred_name_to_idx']
      # p = p[triples[i, 1].item()]
      p = reversed_vocab[triples[i, 1].item()]
      o = triples[i, 2].item()
  ...
  os.system('dot -T%s %s > %s' % (output_format, dot_filename, path))
...
if __name__ == '__main__':
  ...
  print(draw_scene_graph(objs, triples, "./graph.png", orientation='V'))
